...
func (r *OrderHandler) CreateOrder(ctx *gin.Context) {
	...
	if jsonBindingErr := ctx.ShouldBindJSON(&order); jsonBindingErr != nil {
		...
		utils.ErrorResponse(ctx, http.StatusBadRequest, "Invalid request body.", requestErrors)
		ctx.Abort()
		return
	}
	...
	if err := r.repo.CreateSingleOrder(ctx.Request.Context(), &order); err != nil {
		...
		CustomErrors["error"] = err.Error()
		utils.ErrorResponse(ctx, http.StatusBadRequest, "Unable to create a order.", CustomErrors)
		return
	}
...
func (r *OrderHandler) DeleteOrder(ctx *gin.Context) {}
